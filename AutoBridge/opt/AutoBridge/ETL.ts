// GM61 Autobridge Version 2.0008
...
*/
// Command to start the script (testing): deno run --allow-read --allow-net --allow-env --allow-write --allow-run --unstable-kv ETL.ts
/*
Compile on MAC: 
deno compile --allow-read --allow-net --allow-env --allow-write --unstable-kv --allow-run --output washer-uploader ETL.ts
Command to compile on Windows:
deno compile --allow-read --allow-net --allow-env --allow-write --unstable-kv --target x86_64-pc-windows-msvc --allow-run --no-check --output GM61-AutoBridge.exe ETL.ts
 ...
 */
async function sendFile(filepath: string, subfolder: string) {
  try {
...
async function processFolder() {
  ...
  while (true) {
    ...
    for await (const washerFolder of Deno.readDir(folderPath)) {
      if (washerFolder.isDirectory) {
        const washerPath = join(folderPath, washerFolder.name);

        // Iterate over each file in the washer subfolder
        for await (const entry of Deno.readDir(washerPath)) {
          ...
          if (entry.isFile && entry.name.toLowerCase().endsWith(".xml")) {
            ...
            // Check file creation date against startDate if set
            const fileInfo = await Deno.stat(fullPath);
            const fileCreatedDate = fileInfo.birthtime ?? fileInfo.mtime;
            if (startDate && fileCreatedDate && fileCreatedDate < startDate) {
              continue; // skip files before the start date
            }

            // Check if this file has already been processed to avoid duplicates
            const filename = entry.name;
            const existing = await db.get(["processed_files", filename]);
            if (!existing.value) {
              // Log the discovery of a new file
              await writeLog(`Found new file: ${filename} in ${washerFolder.name}`);
              // Attempt to send the file to the API
    ...
    // Wait for 10 seconds before checking the folder again to reduce CPU usage
